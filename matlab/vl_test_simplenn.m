function res = vl_test_simplenn(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'convt'
      output = vl_nnconvt(res(i).x, l.weights{1}, l.weights{2}, ...
        'crop', l.crop, ...
        'upsample', l.upsample, ...
    ...
    case 'pool'
      output = vl_nnpool(res(i).x, l.pool, ...
    ...
    case {'normalize', 'lrn'}
      output = vl_nnnormalize(res(i).x, l.param) ;

    case 'softmax'
      output = vl_nnsoftmax(res(i).x) ;

    case 'loss'
      output = vl_nnloss(res(i).x, l.class) ;

    case 'softmaxloss'
      output = vl_nnsoftmaxloss(res(i).x, l.class) ;

    case 'relu'
      if l.leak > 0, leak = {'leak', l.leak} ; else leak = {} ; end
      output = vl_nnrelu(res(i).x,[],leak{:}) ;

    case 'sigmoid'
      output = vl_nnsigmoid(res(i).x) ;

    case 'noffset'
      output = vl_nnnoffset(res(i).x, l.param) ;

    case 'spnorm'
      output = vl_nnspnorm(res(i).x, l.param) ;

    case 'dropout'
      if testMode
        output = res(i).x ;
      else
        [output, res(i+1).aux] = vl_nndropout(res(i).x, 'rate', l.rate) ;
      end

    case 'bnorm'
      if testMode
        output = vl_nnbnorm(res(i).x, l.weights{1}, l.weights{2}) ;%, 'moments', l.weights{3}
      else
        output = vl_nnbnorm(res(i).x, l.weights{1}, l.weights{2}) ;
      end

    case 'pdist'
      output = vl_nnpdist(res(i).x, l.class, l.p, ...
        'noRoot', l.noRoot, ...
        'epsilon', l.epsilon, ...
  ...
  if info.splitLayer
    ...
    res(i+1).x = output;
    res(pLayer).x = output;
  elseif info.endLayer == 1
    b = info.lossLayer;
    res(b).x = zeros(1,1,236,size(output,4),'single');
    res(b).x(:,:,1:8,:) = gather(output);
  elseif info.endLayer == 2
    b = info.lossLayer;
    res(b).x(:,:,9:236,:) = gather(output);
  else
    res(i+1).x = output;
  end
  ...
  forget = opts.conserveMemory && ~needsBProp && 0;
  if i > 1
    lp = net.layers{i-1} ;
...
if doder
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
      ...
      case 'mp-loss'
        para.loss = vl_dfaces_m_loss(res(1).x,res(i).x, para, l.mtask, res(i+1).dzdx);     
        if p_mode 
            loss = vl_dfaces_p_loss(res(1).x,res(i).x, para, l.mtask, res(i+1).dzdx); 
        else
            loss = para.loss;
        end
        res(i).dzdx = gpuArray( abs(l.mtask.lr1)*loss.task1 + ...
                                abs(l.mtask.lr2)*loss.task2 + ...
                                abs(l.mtask.lr3)*loss.task3 + ...
                                abs(l.mtask.lr4)*loss.task4 + ...
                                abs(l.mtask.lr5)*loss.task5+ ...
                                abs(l.mtask.lr6)*loss.task6+ ...
                                abs(l.mtask.lr7)*loss.task7+ ...
                                abs(l.mtask.lr8)*loss.task8+ ...
                                abs(l.mtask.lr9)*loss.task9+ ...
                                abs(l.mtask.lr10)*loss.task10+...
                                0.1*loss.regularization);  % should be a 1*1*6*batchsize
        para = [];
